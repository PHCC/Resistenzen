...
```{r}
# Start mit leeren Spalten (nötig!?) :
...
for( j in 0:2 ){          # HSC als nur 3 dummy-Variablen für Korrelation&Regression
...
for (i in 1:ResRow ) {                  # Schleife über alle Einträge 
  ...
  for( j in 0:2 ){              # HSC umkodieren als nur 3 dummy-Variablen für Korrelation&Regression:
                                # 0, 1, 2 bleiben
    Resistenzen[i,paste("HSC",j,sep="")] <- if(Resistenzen[i,"HSC.group"] == j) 1  else 0 
  }                             # 3, 4, 5 wieder aufdrüseln
  if(Resistenzen[i,"HSC.group"] == 3){      # 3 = 0 + 1                                      
    Resistenzen[i,"HSC0"]      <- 1
    Resistenzen[i,"HSC1"]      <- 1
  }
  if(Resistenzen[i,"HSC.group"] == 4){      # 4 = 1 + 2
    Resistenzen[i,"HSC1"]      <- 1
    Resistenzen[i,"HSC2"]      <- 1
  }
  if(Resistenzen[i,"HSC.group"] == 5){      # 5 = 0 + 2
    Resistenzen[i,"HSC0"]      <- 1
    Resistenzen[i,"HSC2"]      <- 1
  }
  ### Neue nominale hier dazufügen ###
