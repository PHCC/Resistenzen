...
#LogV <- function(df)     { if(debug){View(df)}      }
```
...
#Resistenzen[116,] Farm 29
#Resistenzen[117,] Farm 31
```
...
Resistenzen["MY.group" ] <- vector(mode="character", length=ResRow)               
Resistenzen["SCC.group"] <- vector(mode="character", length=ResRow)              
Resistenzen["CBC.group"] <- vector(mode="character", length=ResRow)        
Resistenzen["DIA.group"] <- vector(mode="character", length=ResRow)              
### Neue nuerische hier dazufügen ###

for (i in c(1:ResRow)) {                  # Schleife über alle Einträge 
  ...
  Resistenzen[i,"MY.group" ] <- codes[codes["Farm.ID"] == Farm_ID,"Q6.meanMY.cow"               ]              
  Resistenzen[i,"SCC.group"] <- codes[codes["Farm.ID"] == Farm_ID,"Q7.mean.SCC.11mo"            ]   
  Resistenzen[i,"CBC.group"] <- codes[codes["Farm.ID"] == Farm_ID,"Q13a.calvingbox_clean"       ]      
  ### Neue nominale hier dazufügen ###

  Resistenzen[i,"DIA.group"] <- codes[codes["Farm.ID"] == Farm_ID,"Q17.IN_diarrhea.30d" ]       
  ### Neue numerische hier dazufügen ###
}

if(debug){View(Resistenzen)}     # 236 Zeilen (ohne Farm 30)
write.csv(Resistenzen      ,"Resistenzen.csv"      )  # ein Spaltentyp wird unten geändert
```

# Data Frames für die 2 MY-Schichten, als 2 CSV rausschreiben

```{r}
Resistenzen$MY.group <- as.numeric(Resistenzen$MY.group)
if(debug){View(Resistenzen)}
```


```{r}
ResistenzenLE8000 <- Resistenzen[Resistenzen["MY.group"] <= 8000,]  # Less than or Equal to 8000
ResistenzenGT8000 <- Resistenzen[Resistenzen["MY.group"]  > 8000,]  # Greater Than 8000
